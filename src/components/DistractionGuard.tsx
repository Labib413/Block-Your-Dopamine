...
export function DistractionGuard() {
  ...
  const handleAddWebsite = (e: React.FormEvent) => {
    ...
    // Security: Validate URL to prevent XSS and malformed entries
    if (!isValidUrl(newSiteUrl)) {
      addNotification("Security Alert", "Invalid or unsafe website link provided.");
      return;
    }
  ...
  const handleLinkClick = (site: GuardedWebsite) => {
    if (site.is_active) {
      ...
      // Security: Use safeOpen to prevent reverse tabnabbing and validate protocol
      safeOpen(site.url);
    }
  };
