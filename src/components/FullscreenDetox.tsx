  ...
  const handleResourceClick = React.useCallback((res: Resource) => {
    ...
    if (!isIframeSafe(latestRes.url)) {
      ...
      if (!currentWin || currentWin.closed) {
        if (isValidUrl(latestRes.url)) {
          // Security: Use noreferrer to prevent the new page from accessing the opener
          // However, we MUST NOT use noopener because we need the window reference for tracking and focus.
          // Vulnerability mitigated via win.opener = null below.
          const win = window.open(latestRes.url, `byd_resource_${latestRes.id}`, 'width=1000,height=800,noreferrer=yes');
          if (win) {
            // Mitigation for reverse tabnabbing: manually disconnect the opener
            // while retaining the reference in our application state for window tracking.
            try {
              win.opener = null;
            } catch (e) {
              // Silently fail if browser blocks manual opener reset
            }
            ...
            <div className="flex bg-[#111] border-b border-white/10 overflow-x-auto scrollbar-hide shrink-0">
              {openTabs.map(tab => (
                ...
                >
                  ...
                  {tab.type === 'PDF' && (
                    <a 
                      href={isValidUrl(tab.url) ? tab.url : '#'}
                      target="_blank" 
                      rel="noopener noreferrer"
