...
export function Sidebar({ onNavigate, currentView }: { onNavigate: (view: string) => void, currentView: string }) {
  ...
  const { isInstalled, hasNativePrompt, triggerInstall, showModal, setShowModal } = usePWAInstall();
  ...
  return (
    <>
      ...
      >
        ...
        <div className={cn(
          "mt-auto border-t border-white/[0.04] bg-[#0a0a0a] transition-all duration-300 space-y-2.5", 
          isMinimized ? "p-3" : "p-5"
        )}>
          ...
          <motion.button
            ...
            className={cn(
              "w-full flex items-center transition-all duration-300 group relative overflow-hidden bg-gradient-to-r from-[#39FF14]/15 via-[#39FF14]/10 to-transparent border border-[#39FF14]/30 hover:border-[#39FF14]/70 shadow-[0_0_15px_rgba(57,255,20,0.15)] hover:shadow-[0_0_22px_rgba(57,255,20,0.3)] text-[#39FF14]",
              isMinimized ? "justify-center p-3 rounded-2xl" : "justify-between px-3.5 py-2.5 rounded-2xl"
            )}
            title="Install BYD App (PWA)"
          >
            <div className="flex items-center gap-2.5">
              <div className="w-7 h-7 rounded-xl bg-[#39FF14]/20 border border-[#39FF14]/40 flex items-center justify-center shrink-0">
                <Download className="w-3.5 h-3.5 text-[#39FF14] animate-bounce" />
              </div>
              <AnimatePresence initial={false}>
                {!isMinimized && (
                  <motion.div 
            ...
            {!isMinimized && (
              <div className="w-2 h-2 rounded-full bg-[#39FF14] animate-ping opacity-75 shrink-0 mr-1" />
            )}
          </motion.button>
