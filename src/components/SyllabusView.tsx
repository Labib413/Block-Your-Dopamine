...
export function SyllabusView({ subjectId, onBack }: SyllabusViewProps) {
  ...
  useEffect(() => {
    ...
    // Create a Map for O(1) lookups during hydration
    const cloudMap = new Map<string, AcademicChapter>(academicChapters.map(c => [c.id, c]));
    ...
    const baseChapters = defaultNames.map((name) => {
      ...
      const cloudData = cloudMap.get(chapterId);
