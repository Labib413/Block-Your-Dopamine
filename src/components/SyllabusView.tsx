...
export function SyllabusView({ subjectId, onBack }: SyllabusViewProps) {
  ...
  const handleAddResource = (chapterId: string) => {
    ...
    // Security: Validate the provided URL to prevent XSS (javascript:) or unsafe protocols
    if (!isValidUrl(url)) {
      alert("Invalid or unsafe URL provided. Please enter a valid http, https, or other supported link.");
      return;
    }
