...
export function SyllabusView({ subjectId, onBack }: SyllabusViewProps) {
  ...
  useEffect(() => {
    ...
    // OPTIMIZATION: Use a Map for O(1) lookups during hydration
    const cloudDataMap = new Map<string, AcademicChapter>(academicChapters.map(c => [c.id, c]));
    ...
    const baseChapters = defaultNames.map((name) => {
      ...
      const cloudData = cloudDataMap.get(chapterId);
