...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // Bolt Optimization: Use Map for O(1) lookups during progress calculation
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          // Bolt Optimization: Replace O(N^2) chapter merging with O(N) using Maps
          const cloudChapters = results.academicChapters.data as AcademicChapter[];
          const cloudMap = new Map<string, AcademicChapter>(cloudChapters.map(c => [c.id, c]));
