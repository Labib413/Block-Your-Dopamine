...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // Bolt Optimization: Use a Map for O(1) lookups instead of O(N) array finds
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          ...
          // Bolt Optimization: Pre-calculate Maps for O(1) lookups during merge
          const cloudChapterMap = new Map<string, any>(cloudChapters.map(c => [c.id, c]));
