...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // PERFORMANCE OPTIMIZATION: Use a Map for O(1) lookups during subject progress calculation.
  // This prevents O(S * C * N) complexity (Subjects * Chapters in Syllabus * Total Chapters).
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          ...
          // PERFORMANCE OPTIMIZATION: Use Maps for O(1) lookups during chapter merging
          const cloudChapterMap = new Map<string, any>();
          cloudChapters.forEach((c: any) => cloudChapterMap.set(c.id, c));

          const localChapterMap = new Map<string, AcademicChapter>();
          prev.academicChapters.forEach(c => localChapterMap.set(c.id, c));
