...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // PERFORMANCE OPTIMIZATION: Use a Map for O(1) lookup during progress calculation
  const chapterMap = new Map<string, AcademicChapter>();
  chapters.forEach(c => {
    // Ensure "latest-wins" logic using _timestamp, falling back to array order
    const existing = chapterMap.get(c.id);
    if (!existing || (c._timestamp || 0) >= (existing._timestamp || 0)) {
