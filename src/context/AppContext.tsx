...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // PRIMARY FIX: Use Map for O(1) lookups to avoid O(N^2) bottlenecks in progress calculation
  ...
  const updatedSubjects = subjects.map(s => {
    ...
    // THE MASTER CALCULATION LOOP (Optimized with Map lookups):
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          ...
          // PRE-COMPUTE MAPS for O(1) lookups during merge
          const cloudChaptersMap = new Map<string, any>();
          for (const c of cloudChapters) cloudChaptersMap.set(c.id, c);

          const localChaptersMap = new Map<string, AcademicChapter>();
          for (const c of prev.academicChapters) localChaptersMap.set(c.id, c);
