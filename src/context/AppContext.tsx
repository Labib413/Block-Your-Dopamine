...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // BOLT OPTIMIZATION: Use a Map for O(1) lookups instead of O(N) .find() inside the loop.
  // This reduces complexity from O(S * C) to O(C + S * C_avg) where S is subjects and C is total chapters.
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          ...
          // BOLT OPTIMIZATION: Use Maps for O(1) lookups during merging to avoid O(N^2) complexity.
