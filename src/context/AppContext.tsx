...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // We build a Map directly for O(1) lookups later
  ...
  const updatedSubjects = subjects.map(s => {
    ...
    // Optimized with Map lookup instead of .find() inside the loop
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          ...
          // Optimization: Build Maps for O(1) lookups during merge
