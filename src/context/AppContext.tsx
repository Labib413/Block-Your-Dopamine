...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // PERFORMANCE OPTIMIZATION: Use Map for O(1) lookups instead of O(N) array finds
  // This also naturally handles deduplication
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          ...
          const cloudMap = new Map<string, AcademicChapter>(cloudChapters.map((c: AcademicChapter) => [c.id, c]));
