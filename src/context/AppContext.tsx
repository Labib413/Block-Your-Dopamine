...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // OPTIMIZATION: Use a Map for O(1) lookups instead of O(N) .find() inside nested loops.
  // This reduces complexity from O(Subjects * ChaptersPerSubject * TotalChapters) to O(TotalChapters + TotalSyllabusNodes).
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          ...
          // OPTIMIZATION: Use Maps for O(1) lookups during chapter merging to avoid O(N^2) complexity.
