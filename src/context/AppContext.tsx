...
const calculateAllSubjectsProgress = (chapters: AcademicChapter[], userId: string | null) => {
  ...
  // PRIMARY FIX: Use Map for O(1) lookups and implicit deduplication to prevent "ghost" data
    ...
    try {
      ...
      setState(prev => {
        ...
        if (results.academicChapters?.data) {
          const cloudChapters = (results.academicChapters.data || []) as AcademicChapter[];
          const cloudChapterMap = new Map<string, AcademicChapter>();
          cloudChapters.forEach(c => { if(c && c.id) cloudChapterMap.set(c.id, c); });

          const localChapterMap = new Map<string, AcademicChapter>();
          prev.academicChapters.forEach(c => { if(c && c.id) localChapterMap.set(c.id, c); });
          ...
          const mergedChapters: AcademicChapter[] = defaultChapters.map(defaultCh => {
            ...
            const localTimestamp = (localCh as any)?._timestamp || 0;
            const cloudTimestamp = (cloudCh as any)?._timestamp || 0;

            if (localTimestamp > cloudTimestamp && localCh) return localCh;
            if (cloudCh) return {
              ...
              _timestamp: (cloudCh as any)?._timestamp || 0
            } as AcademicChapter;
